...
export async function POST(
  ...
  for (const task of tasks) {
    ...
    const alreadyAnnotatedIds = task.annotations.map((a) => a.userId).filter((id): id is string => Boolean(id));

    // Find someone from the project pool who hasn't annotated this task yet
    const candidates = allProjectUserIds.filter(
      (uid: string) => !alreadyAnnotatedIds.includes(uid) && !currentAssignedIds.includes(uid)
    );

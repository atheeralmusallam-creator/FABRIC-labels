    ...
    <span className="text-[10px] text-emerald-400 tracking-wider uppercase opacity-80">
      Powered By <span className="font-semibold">FABRIC</span>
          ...
          <nav className="flex items-center gap-6 text-sm text-[var(--text-secondary)]">
            <Link href="/dashboard" className="text-[var(--text-primary)] font-medium">Organizations</Link>
            {user.role === "ADMIN" && <Link href="/admin/users" className="hover:text-[var(--text-primary)] transition-colors">Users</Link>}
            <UserMenu user={user} />
          </nav>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex items-center justify-between mb-8">
          ...
          {user.role !== "ANNOTATOR" && (
            <Link href="/organizations/new" className="flex items-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 shadow-md shadow-emerald-500/20 text-[var(--text-primary)] text-sm font-medium px-4 py-2 rounded-lg transition-colors">
        ...
        {organizations.length === 0 ? (
          <div className="text-center py-24 border border-dashed border-[var(--border)] rounded-xl">
            ...
            <p className="text-[var(--text-secondary)] text-lg font-medium">No organizations yet</p>
            ...
            {user.role !== "ANNOTATOR" && (
              <Link href="/organizations/new" className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 shadow-md shadow-emerald-500/20 text-[var(--text-primary)] text-sm font-medium px-5 py-2.5 rounded-lg transition-colors">Create Organization</Link>
            )}
          </div>
        ) : (
                ...
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="text-base font-semibold text-[var(--text-primary)] group-hover:text-indigo-300 transition-colors truncate">{org.name}</h2>

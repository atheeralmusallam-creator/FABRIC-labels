    ...
    <div className="space-y-5">
      <div className="bg-[var(--bg-secondary)] border border-[var(--border)] rounded-xl p-4">
        <div className="flex flex-wrap gap-4 text-xs text-[var(--text-muted)]">
          {data.id && (
            <span>
              ID: <span className="text-[var(--text-secondary)]">{data.id}</span>
            </span>
          )}
          {data.dataset && (
            <span>
              Dataset: <span className="text-[var(--text-secondary)]">{data.dataset}</span>
            </span>
          )}
        </div>
      </div>
              ...
              <button
                ...
                className={`text-left rounded-lg border px-4 py-3 text-sm transition-all ${
                  active
                    ...
                    : "border-[var(--border)] bg-[var(--bg-primary)] text-[var(--text-secondary)] hover:border-emerald-500/50"
                }`}
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div>
          ...
          <textarea
            ...
            rows={4}
            className="w-full bg-[var(--bg-primary)] border border-[var(--border)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] outline-none focus:border-emerald-500 resize-none"
          />
        </div>
